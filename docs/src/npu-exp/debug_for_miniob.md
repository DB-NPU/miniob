...
- program：该选项指编译出来的可执行文件的路径,本文中路径为"${workspaceFolder}/**build_debug**/bin/observer",之所以于官方文档不同，是因为使用命令**bash build.sh**编译项目时，会生成 debug 版的 MiniOB，源文件会存储在 build_debug 目录下。
...
这样我们就可以使用 gdb 命令来调试 core 文件了。 可以输入 `bt` 命令，查看当前的调用堆栈，位于调用堆栈最上方的函数，即是发生段错误的位置。